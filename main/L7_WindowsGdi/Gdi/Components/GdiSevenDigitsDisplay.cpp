...
#include "../../Common/Components/Tm1637/Tm1637DeviceModel.hpp"

const int LENGTH = 10; // Per digit
const int WIDTH = 20; 

GdiSevenDigitsDisplay::GdiSevenDigitsDisplay(
    GdiScreen& gdiScreen, Tm1637DeviceModel& tm1637DeviceModel, bool hasColon, int x, int y)
...
void GdiSevenDigitsDisplay::Update(HDC* hdc)
{
	...
	RECT rect{ _x, _y, _x + D(LENGTH * (_tm1637DeviceModel.GetNumberOfDigits() + 1) + 5), _y + D(WIDTH)}; // +1 for auxiliary segment
	...
	char output[32]; 
    _tm1637DeviceModel.GetStringRepresentation(output);
